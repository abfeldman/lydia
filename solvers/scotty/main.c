...
#define PROMPT "scotty> "
#define MODULE "scotty"
...
int main(int argc, char **argv)
{
    ...
    tv_dnf_flat_kb model = tv_dnf_flat_kbNIL;
    ...
    if (!fscan_tv_dnf_flat_kb(modelfile, &model)) {
        fprintf(stderr,
    ...
    if (tv_dnf_flat_kbNIL == model) {
        fprintf(stderr, "Nothing to diagnose.\n");
        rc = EXIT_FAILURE;
        goto exit;
    }
    if (tv_term_listNIL == to_tv_dnf(model->constraints)->terms) {

...
def create_default_input(test):
    tmp_filename = tempfile.mktemp('-tmp.input',
                                   'cdas-test-%s-' % os.path.basename(test.model),
                                   '/tmp/')
...
def run_cdas_command(test):
    ...
    starttime = time.time()
    pipe = subprocess.Popen([cdas, test.model, test.observation],
                            stdin=infile,
...
if __name__ == "__main__":
    ...
    run_tests(srcdir, run_cdas_command)

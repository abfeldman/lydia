...
int cdas_is_sat(const_tv_cnf sd,
...
int cdas_enable_observation(ltms tms, const_tv_term term)
{
...
int cdas_assume_health(ltms tms,
                       ...
                       array assignments)
{
    ...
    for (ix = 0; ix < assignments->sz; ix++) {
        ...
        ltms_enable_assumption(tms, tv_cache->a_to_v[lit->var], lit->sign);
        if (ltms_has_contradiction(tms)) {
            return 0;
        }
    }
...
void cdas_retract_observation(ltms tms, const_tv_term term)
{
    register unsigned int ix;
...
void cdas_retract_health(ltms tms,
                         const_tv_variables_cache tv_cache,
                         array assignments)
{
    register unsigned int ix;

...
""" Script for running gotcha tests. """
...
def create_default_input(test):
    tmp_filename = tempfile.mktemp('-tmp.input',
                                   'gotcha-test-%s-' % os.path.basename(test.model),
                                   '/tmp/')
...
def run_gotcha_command(test):
    ...
    pipe = subprocess.Popen([gotcha, test.model, test.observation],
                            stdin=infile,
...
if __name__ == "__main__":
    ...
    run_tests(srcdir, run_gotcha_command)
